...
class TravelPlanWithMostPlaces {
    ...
    public void calculateShortestPath(Place start, double startTime) {
        ...
        PriorityQueue<Place> unsettledPlaces = new PriorityQueue<Place>(new Comparator<Place>(){
            @Override
            public int compare(Place c1, Place c2){
                if(c1.getCloseTime() == c2.getCloseTime()){
                    return 0;
                }
                return c1.getCloseTime() < c2.getCloseTime() ? -1 : 1;
            }
        });
