...
@Service
class TravelPlanPerDayService {
    public PlaceGraphBuilderService graphBuilder;
    public TripPlanDetailsPerDay tripPlanDetailsPerDay;
    List<PlaceVisitDetails> visits = tripPlanDetailsPerDay.visits();

    @Autowired
    public TravelPlanPerDayService(PlaceGraphBuilderService graphBuilder) {
    ...
    public void calculateShortestPath(Place start, String dateTime, Time startTime, Time endTime) {
        ...
        if (!graphBuilder.travelInOpenTime(startPlaceDetails, graphBuilder.timeToMinutes(startTime), dateTime)) {
        ...
        while (!unVisitedPlaces.isEmpty()) {
            ...
            if (!graphBuilder.travelInOpenTime(currentPlaceDetails, arrivalTimeInMinutes, dateTime) || leaveTimeInMinutes > endTimeInMinutes) {
                continue;
            }
    ...
    public void autoPath (List<Place> places, String dateTime, Time startTime, Time endTime){
        ...
        for(Place place : places){
            ...
            // If the place is not open or if the stay duration exceeds the limit, skip this place
            if (!graphBuilder.travelInOpenTime(prevPlaceVisitDetails, arrivalTimeInMinutes, dateTime) || leaveTimeInMinutes > graphBuilder.timeToMinutes(endTime)) {
